...
let login = async (req, res) => {
  try {
    ...
    if (comparePassword) {
      ...
      //await newToken.save();
      res.send({ success: true, message: "success", comparePassword: comparePassword, token: accessToken }); 
    }
...
function authenticateToken(req, res, next) {
  ...
  //fetch user data for authenticate user
  // let docs = model.userModel.find({ email: req.user.email });
  // res.json(docs);
}
